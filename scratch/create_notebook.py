...
NOTEBOOK_PATH = "C:/Users/user/Desktop/서울대/eda_image_integration.ipynb"

# Define cells programmatically to avoid any quoting issues
cells = []
...
# Cell 4: TextEmbeddingAligner code
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {"trusted": True},
...
# Cell 6: Spatial3DOwlViTTrajectoryExtractor code
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {"trusted": True},
...
cells.append({
    ...
    "source": [
        ...
        "        else:\n",
        "            similarities = [0.05, 0.30, 0.08]\n",
        "            \n",
        "        current_group = [1]\n",
        "        for idx, dist in enumerate(similarities):\n",
        "            frame_id = idx + 2\n",
        "            if dist > threshold:\n",
        ...
        "        \n",
        "    def verify_crop_identities(self, image_paths, features, threshold=0.35):\n",
        "        identity_jumps = []\n",
        "        if not HAS_CLIP or self.model is None:\n",
        "            return identity_jumps\n",
        "            \n",
        "        successful_features = [f for f in features if f['status'] == 'success']\n",
        "        if len(successful_features) < 2:\n",
        "            return identity_jumps\n",
        "            \n",
        "        embeddings = {}\n",
        ...
        "                    inputs = self.processor(images=crop_img, return_tensors='pt').to(self.device)\n",
        "                    with torch.no_grad():\n",
        "                        feat = self.model.get_image_features(**inputs)\n",
        "                    feat = feat / feat.norm(dim=-1, keepdim=True)\n",
        "                    embeddings[fid] = feat.cpu().numpy()[0]\n",
        "                    \n",
        ...
        "            if dist > threshold:\n",
        "                identity_jumps.append((fid_a, fid_b, dist))\n",
        "                \n",
        "        return identity_jumps"
    ]
})

# Cell 9: Markdown for Corrected Panning/Zoom rules
cells.append({
    "cell_type": "markdown",
    "metadata": {},
    "source": [
...
# Cell 10: build_3d_spatial_auxiliary_hint code
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {"trusted": True},
    ...
    "source": [
        ...
        "    identity_jumps = divider.verify_crop_identities(image_paths, features, threshold=0.35)\n",
        "    \n",
        "    hints = []\n",
        ...
        "    \n",
        "    hints.append(f'- 장면 분할 정보 (CLIP 인접 거리 dist=1-sim: {[round(s, 2) for s in similarities]}):')\n",
        ...
        "    \n",
        "    if identity_jumps:\n",
        "        hints.append(f'- 피사체 추적 정밀 검사 (CLIP Crop Identity Jump 감지):')\n",
        "        for fid_a, fid_b, dist in identity_jumps:\n",
        "            hints.append(f'  * [경고] Image {fid_a}와 Image {fid_b} 사이에서 피사체 일관성 유실 감지! (CLIP 거리 {dist:.3f} > 임계값 0.35)')\n",
        "            hints.append(f'  * (추적 도중 다른 대상이나 배경으로 바운딩 박스가 튀었을 가능성(Tracker Drift)이 높습니다. 이 구간의 궤적 비교는 제외 권장.)')\n",
        "        hints.append('')\n",
        "    \n",
...
# Cell 11: Markdown for holdout_300 evaluation
cells.append({
    "cell_type": "markdown",
    "metadata": {},
    "source": [
...
# Cell 12: evaluate_holdout_consistency code
cells.append({
    "cell_type": "code",
    "execution_count": None,
    "metadata": {"trusted": True},
