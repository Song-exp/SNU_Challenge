...
def parse_model_output(output_text):
    ...
    파싱 우선순위: ① <ANSWER>[..]</ANSWER> 태그 (v4_story_cot 학습 형식, 마지막 태그 채택)
                  ② 마지막 대괄호 그룹 (기존 프롬프트 호환 — CoT 중간 대괄호에도 최종 답만)
    """
    try:
        tags = re.findall(r"<ANSWER>\s*(\[[^\]]*\])\s*</ANSWER>", output_text, re.S)
        if tags:
            seg = tags[-1]
        else:
            s = output_text.rfind("[")
            e = output_text.find("]", s) if s != -1 else -1
            seg = output_text[s : e + 1] if (s != -1 and e != -1) else None
        if seg is not None:
            result = ast.literal_eval(seg)
