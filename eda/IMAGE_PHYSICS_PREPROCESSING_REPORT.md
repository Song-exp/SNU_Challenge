...
* **도입 배경 및 학술적 근거 (Theoretical & Engineering Rationale)**:
  비디오 프레임 순서 정렬 과제에서 Qwen2-VL과 같은 Vision-Language Model(VLM)은 입력 프레임들과 자연어 캡션 간의 고차원적 시각-언어 맥락을 추론하는 데 우수한 성능을 보입니다. 그러나 VLM만을 활용하여 프레임 간 저차원 시각 임베딩의 연속성 및 단절성을 정량화하는 데는 다음과 같은 구조적 한계가 존재합니다.

  1. **연속성 편향(Continuity Bias)**: 사전 학습된 VLM은 입력 프레임들을 단일 시퀀스로 강제 해석하려는 경향이 있어, 카메라 앵글이 완전히 전환되는 하드 컷(Hard Cut) 시점을 명확히 식별하지 못합니다.
  2. **저차원 시각 임베딩 거리의 수치화 불가**: VLM은 추가 프롬프트나 정량적 가이드 없이는 프레임 간 픽셀/피처 수준의 미세한 유사도 차이를 스스로 수치화할 수 없습니다.

  이러한 VLM의 한계 상황에서 장면 분할(Global Scene Partitioning)을 거치지 않고 객체 추적기(Tracker)를 이미지 시퀀스 전체에 무작위로 적용할 경우, 완전히 서로 다른 두 장면 사이에서 대상 피사체의 궤적 좌표가 물리적 인과관계 없이 강제로 연결됩니다. 이는 $X, Y$ 중심점 좌표와 $Area$ 크기 변화율 등의 기하학적 수치를 심각하게 왜곡하고 물리 힌트 정보를 오염시키는 결과를 초래합니다.

  따라서 본 프로젝트에서는 비전 모델의 저차원 시각 유사도 인지 한계를 극복하고 물리 힌트의 신뢰성을 확보하기 위해, 추론 파이프라인 전처리 단계에 `CLIP (ViT-B/32)` 임베딩의 코사인 거리를 활용한 글로벌 장면 분할 필터를 도입하였습니다.
...
> [!WARNING]
> **깊이 모델의 수치 왜곡 한계 극복**  
> 단안 깊이 모델은 scale-shift invariant 손실함수로 학습되어 프레임별로 미세한 오프셋(이동 변위)이 달라 수치가 왜곡되는 현상이 실측되었습니다. 이를 극복하기 위해 최종 검증 단계에서는 깊이 정보 대신 **이미지 평면 상의 바운딩 박스 면적 비율($R_{\text{bbox}} = \text{객체 면적} / \text{전체 이미지 면적}$)**을 절대 기준으로 삼아 줌인(면적 증가), 줌아웃(면적 감소)을 왜곡 없이 100% 안정적으로 검출합니다. (실측 검증 시 **66.7%의 높은 줌 판단 일치율** 기록)
...
## 3. 다차원 모호성 연계 분류 체계 및 파라미터 분기 전략 (Multi-dimensional Ambiguity Matrix)

실생활 비디오 데이터셋은 이미지의 시각적 변화 난이도와 자연어 설명 문장의 구조적 모호성이 다차원적으로 얽혀 있습니다. 본 프로젝트에서는 이 두 성질을 각각 계량화하여 **"다차원 모호성 매트릭스"**를 설계하고, 유형별로 최적의 추론 파라미터(Chunk 크기, Likelihood K값 등)를 다르게 푸는 **파라미터 분기 전략(Parameter Routing)**을 적용했습니다.

### 📊 A. 이미지 모호성 분류 (Adjacent MSE 프레임 분석기)
이미지 간의 공간적 변화량을 정량적으로 계산하여 데이터셋을 시각적 난이도에 따라 자동 분류합니다.
1. **1단계: 시각적 노이즈 제거 및 경량화**
   * 모든 입력 프레임을 흑백(Grayscale) 이미지로 변환한 후 가로 32px × 세로 32px 크기로 급격히 축소합니다.
   * 이를 통해 색상이나 미세 조명 노이즈를 배제하고 오직 전체적인 **구도, 밝기 덩어리, 형태의 레이아웃 변화**에만 집중합니다.
2. **2단계: 인접 프레임 간 픽셀 차이 연산 (Adjacent MSE)**
   * 시간축상 인접한 프레임 쌍(1-2, 2-3, 3-4)에 대해 동일 위치 픽셀 값의 평균 제곱 오차(Mean Squared Error)를 연산합니다.
   * 3번의 인접 비교 결과의 평균값($\text{Avg\_MSE}$)을 해당 비디오의 **시각적 변화량 점수**로 채택합니다.
3. **3단계: 임계값(Threshold = 800) 기반 데이터 분기**
   * **Fine-grained (미세 움직임, $\text{Avg\_MSE} < 800$)**: 카메라 구도와 배경이 고정된 상태에서 손동작이나 사물 위치만 미세하게 변하는 고난이도 비주얼 데이터.
   * **Scene Cut (장면 전환, $\text{Avg\_MSE} \ge 800$)**: 컷 전환이 일어나거나 크로즈업 숏이 삽입되어 프레임 간 시각적 변화량이 폭발하는 데이터.

---

### 📝 B. 문장 모호성 분류 체계 및 모호성 지수 (Ambiguity Index, AI)
자연어 문장이 비디오의 순서를 맞추는 데 얼마나 결정적인 시간적/구조적 단서를 주는지 수학적으로 정량화합니다.

#### 1. 문장 모호성 5단계 심층 분류 체계 (Deep Taxonomy)
* **1차 기준 (절의 개수)**: 단일 절(주어-동사 1개)인가, 다중 절(2개 이상)인가?
* **2차 기준 (문법적 단서)**: 시간 접속사 유형(순차 vs 동시), 동사 성격(경계 동작 vs 상태 지속), 전치사 유무.

| 단계 | 카테고리명 | 모호성 수준 | 판단 기준 (동작 및 문법 구조) | 직관적 예시 |
| :---: | :--- | :---: | :--- | :--- |
| **1** | **Explicit-Sequential** | 매우 낮음 | 명시적 시간 선후 접속사 사용 (`then`, `before`, `after`, `finally` 등) | A가 ~하고, 그 다음에(then) B가 ~한다. |
| **2** | **Implicit-Sequential** | 낮음 | 시간 접속사 없이 콤마(`,`)나 `and`로 사건을 순차 나열 | A가 ~하고, B가 ~하고, C가 ~한다. |
| **3** | **Implicit-Trajectory** | 중간 | 단일 동작에 공간/방향 전치사(`down`, `up`, `towards` 등)로 궤적 제시 | 소년이 스케이트보드를 타고 길 아래로(down) 내려간다. |
| **4** | **Single-Action** | 높음 | 수식어 없는 단일 동작만 묘사 (VLM의 미세 포즈 추론 의존) | 여성이 수건으로 얼굴을 닦는다(wipe). |
| **5** | **Static-State** | 매우 높음 | 동시 지속 접속사(`while`, `as`) 혹은 정적인 상태만 묘사 | 남자가 제트스키에 끌려 수상스키를 타는 중이다(while). |

#### 2. 문장 모호성 지수 (Ambiguity Index, AI) 산출 공식
$$\text{AI} = 1.0 - (S_{\text{temp}} + S_{\text{aspect}} + S_{\text{motion}})$$
* **시간적 제약 ($S_{\text{temp}}$)**: 명시적 접속사 존재 시 $+1.0$, 순차 나열 시 $+0.5$, 동시성 접속사(`while`, `as`) 존재 시 $-0.3$.
* **동작의 경계성 ($S_{\text{aspect}}$)**: 시작과 끝이 뚜렷한 동작 동사의 비율에 비례하여 가점.
* **물리적 궤적 ($S_{\text{motion}}$)**: 이동 방향 전치사 개당 $+0.4$ 가점.
* **해석**: AI가 `0.0`에 가까울수록 텍스트 정보가 명확하고, `1.0`을 초과할수록 텍스트 정보가 전무하여 순수 비주얼에 의존해야 함을 의미합니다.

---

### 🧩 C. 교차 모호성 매트릭스 및 파라미터 분기 (Routing)
시각 모호성과 문장 모호성을 2차원 교차 매트릭스로 결합하여 문제 유형을 분류하고, 각 유형에 최적화된 **추론 파라미터 라우팅**을 적용합니다.
...
```
            ...
            +--------------------+--------------------+
   낮음     |     유형 I         |     유형 II        |
  (Scene    | (매우 명확함)      | (비주얼 기반 추론)  |
   Cut)     |  Greedy 디코딩     |   우도 K2 TTA      |
 이미지     +--------------------+--------------------+
 모호성     |     유형 III       |     유형 IV        |
   높음     | (물리 인과 대조)   | (최고 난이도 데이터) |
 (Fine-grained)|  우도 K2 TTA    |   우도 K4 TTA      |
            +--------------------+--------------------+
```

* **파라미터 라우팅 예시 (Parameter Routing)**:
  * **유형 I (이미지 MSE 800 이상 & 문장 AI 낮음)**: 시각 변화가 뚜렷하고 지문 순서가 명시적이므로 연산 비용이 낮은 **Greedy 디코딩**으로 라우팅하여 추론 지연 시간을 극적으로 세이브.
  * **유형 IV (이미지 MSE 800 미만 & 문장 AI 높음)**: 구도가 고정된 미세 움직임 상태에서 텍스트 정보가 모호한 최고 난이도 유형. VLM의 예측 노이즈를 극한으로 상쇄하기 위해 **가장 정밀한 [우도 K4 TTA (4회전 앙상블)]**과 고해상도 연산 슬롯을 강제 할당.
  * **유형화 한계 데이터 처리 (Outliers & Failure cases)**: 매트릭스의 경계선에 걸쳐 유형화에 실패하거나 오분류 리스크가 존재하는 10%의 예외 데이터셋의 경우, **인위적인 증강(Augmentation) 가중치(hard_perm 및 augw 배율 조정)**를 부여하여 사전 학습 단계에서 강인하게 적응시켰습니다.

---

## 4. 다중 객체 Identity 혼선 차단 및 트래커 오탐지 (Tracker Drift) 방어
...
## 5. 최종 모델 설계 시 이미지 물리 기법의 '기각 사유' 및 '반영 전략'
...
## 7. 이미지 물리 전처리 및 분석 방법론 개발 타임라인 (Timeline)
