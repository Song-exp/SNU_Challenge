...
* **결론**: 전체 이미지의 80% 이상이 `640x360` 이하의 크기를 가지고 있어, 학습 및 추론 시 이미지 입력 크기를 무리하게 키울 필요가 전혀 없으며, 입력 크기를 `max_pixels = 384 * 512` 또는 `224x224`로 고정하는 것이 연산 속도 확보에 가장 유리함을 실증했습니다.
...
### A. 안엄격(느슨한) 판단 기준 (Loose Cut Algorithm)
...
### B. 안엄격 기준에 따른 최종 장면 전환 횟수(0~3회) 분포
...
### D. 유니온-파인드(Union-Find) 기반 장면 경계 분할 군집화
...
### C. 기술적 문제 해결 성과 (Troubleshooting)
...
### D. 하드코딩 및 과적합 방지 검증 설계 (Anti-Overfitting & Robustness)
