...
def main():
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Extracting Features"):
        ...
        # 컷 개수 맵핑
        cuts = map_similar_pairs_to_cuts(similar_clip_pairs)
    ...
    # 결과 최종 테이블 병합 (로컬 GUI 검수기와의 호환을 위해 컬럼명을 Max, Mean으로 매핑)
    res_df = pd.DataFrame({
        'Id': ids,
    ...
    print(f"\n💾 전체 데이터 라벨링 및 피처 CSV 저장 성공: '{OUTPUT_FILE}'로 저장 완료!")
