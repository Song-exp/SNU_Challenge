...
### 1.2 시간순 인접 프레임 간 픽셀 오차 분석 (Sequential Adjacent MSE Analysis)
셔플된 상태가 아닌, 정답(`Answer`) 기준으로 원래의 시간 순서(1번 ➡️ 2번 ➡️ 3번 ➡️ 4번 프레임)로 복원하여 인접 프레임 간의 오차(Mean Squared Error, Grayscale 64x64 기준)를 전수 분석했습니다. (총 28,605개 전이 구간 대상)
* **픽셀 오차값(MSE) 분포**:
  * 최소값 (Min): `0.00`
  * 25% 백분위수: `2,446.72`
  * 50% 백분위수 (중앙값): `4,320.55`
  * 75% 백분위수: `6,900.32`
  * 최대값 (Max): `64,206.21`
* **일반화된 장면 전환 임계 구간 (Threshold Range)**:
  기존 단일 고정 임계값(800)의 한계와 과적합(Overfitting) 리스크를 보완하고 데이터에 일반화될 수 있는 물리적 기준을 찾기 위해, 오차 분포의 계곡(Valley) 영역 분석을 설계 및 수행했습니다.
  * **동일 장면 (Same Scene)**: `MSE < 1,000` (전이 구간의 6.74% 차지. 확실히 컷 전환이 없음)
  * **모호한 전이 (Transition Boundary)**: `1,000 <= MSE <= 2,000` (전이 구간의 12.35% 차지. 카메라 줌, 페이드, 빠른 모션 영역)
  * **장면 전환 (Scene Cut)**: `MSE > 2,000` (전이 구간의 80.91% 차지. 확실히 다른 배경 씬으로 전환)

### 1.3 비디오 연출 유형 세분화 (Video Production Types)
도출된 장면 전환 임계 구간(기준값 1,200)을 대입하여, 각 비디오가 지니는 **장면 전환 횟수(0회~3회)**에 따라 전체 비디오의 연출 유형을 4가지로 세분화했습니다.
* **유형 0 (장면 전환 0회 - 롱테이크 미세 행동)**: **1.92%** (183개)
  * 처음부터 끝까지 컷이 단 한 번도 바뀌지 않는 순수 미세행동 데이터입니다. (해상도가 극도로 중요함)
* **유형 1 (장면 전환 1회)**: **3.48%** (332개)
  * 장면이 중간에 딱 한 번 다른 씬으로 바뀌는 비디오 (2개 씬 존재)
* **유형 2 (장면 전환 2회)**: **14.00%** (1,335개)
  * 장면이 두 번 바뀌어 총 3개의 다른 씬이 존재하는 비디오
...
1. **정확도 차이의 통계적 무의성 (McNemar's Test)**:
   * 200개 샘플 중 Grid_1~3(34개 정답, 166개 오답)과 Grid_4(32개 정답, 168개 오답)의 2개 차이는 통계적으로 유의미한 수준이 아닙니다.
...
2. **서브셋 분석 한계 및 성능 착시 (Aggregate vs. Fine-grained)**:
   * 이미지 EDA 결과, 전체 데이터셋 중 프레임 간 미세한 동작 차이만 존재하는 "미세 행동 변화(유형 0 - 0회 전환)" 데이터의 비중은 **1.92%**에 불과합니다.
   * 실제 본 200개 검증 셋에 대해 검사한 결과, 미세 행동 변화 샘플은 **0개 (0.0%)** 포함된 것으로 나타났습니다.
   * 즉, 검증 셋 전체가 장면 전환(유형 1~3, 1회 이상 전환) 샘플로만 구성되어 다수 집단이 평균 정확도를 지배해 버렸으며, 이로 인해 해상도가 정말 중요한 미세 행동 영역에서의 성능 편차가 통계에서 완전히 은폐(착시)된 상태입니다.
