...
셔플된 상태가 아닌, 정답(`Answer`) 기준으로 원래의 시간 순서(1번 ➡️ 2번 ➡️ 3번 ➡️ 4번 프레임)로 복원하여 인접 프레임 간의 오차(Mean Squared Error, Grayscale 64x64 기준)를 전수 분석했습니다. (총 3,000개 전이 구간 샘플링)
* **픽셀 오차값(MSE) 분포**:
  * 최소값 (Min): `0.00`
  * 25% 백분위수: `2,374.01`
  * 50% 백분위수 (중앙값): `4,229.38`
  * 75% 백분위수: `6,787.95`
  * 최대값 (Max): `64,202.02`
* **일반화된 장면 전환 임계 구간 (Threshold Range)**:
  정현 님이 지적하신 단일값(800)의 과적합 리스크를 배제하고, 오차 분포의 계곡(Valley) 영역을 탐색하여 일반화된 구간 기준을 도출했습니다.
  * **동일 장면 (Same Scene)**: `MSE < 1,000` (전이 구간의 7.53% 차지. 확실히 컷 전환이 없음)
  * **모호한 전이 (Transition Boundary)**: `1,000 <= MSE <= 2,000` (전이 구간의 12.04% 차지. 카메라 줌, 페이드, 빠른 모션 영역)
  * **장면 전환 (Scene Cut)**: `MSE > 2,000` (전이 구간의 80.43% 차지. 확실히 다른 배경 씬으로 전환)
...
2. **서브셋 분석 한계 및 성능 착시 (Aggregate vs. Fine-grained)**:
   * 이미지 EDA 결과, 전체 데이터셋 중 프레임 간 미세한 동작 차이만 존재하는 "미세 행동 변화(유형 0 - 0회 전환)" 데이터의 비중은 **2.70%**에 불과합니다.
