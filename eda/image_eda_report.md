...
2. **서브셋 분석 한계 및 성능 착시 (Aggregate vs. Fine-grained)**:
   * 이미지 EDA(1.5절) 결과, 전체 데이터셋 중 프레임 간 미세한 동작 차이만 존재하는 "미세 행동 변화(Fine-grained Still Action, MSE < 800)" 데이터의 비중은 **1.3%**에 불과합니다.
   * 실제 본 200개 검증 셋에 대해 검사한 결과, 미세 행동 변화 샘플은 **0개 (0.0%)** 포함된 것으로 나타났습니다.
   * 즉, 검증 셋 전체가 장면 전환(Scene Cut, MSE >= 800) 샘플로만 구성되어 다수 집단이 평균 정확도를 지배해 버렸으며, 이로 인해 해상도가 정말 중요한 1.3%의 미세 행동 영역에서의 성능 편차가 통계에서 완전히 은폐(착시)된 상태입니다.
