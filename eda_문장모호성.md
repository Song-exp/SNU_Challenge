...
### 📂 [발표용 슬라이드 구성안] 문장 모호성 분류 및 AI Score
* **Slide 1: [기존 한계] 단순 텍스트 분류와 하드코딩의 오버피팅 리스크**
  - *핵심*: 기존 5단계 분류 체계는 데이터가 한쪽으로 쏠리는 클래스 희소성 문제(가장 적은 클래스 19개 샘플)로 모델 학습이 불가능했습니다. 
  - *해결*: 절(Clause)의 개수를 기준으로 한 상호배타적 3단계 통사 파티션으로 개편하여 Train 셋 데이터 불균형 리스크를 완벽히 소멸시켰습니다.
* **Slide 2: [제안 모델] 3대 통사 파티션 × 7대 직교 플래그 하이브리드 설계**
  - *핵심*: 문장의 문법적 뼈대(통사 구조)와 문맥에 녹아있는 시간 지시 정보(의미론)를 분리하여 다차원 벡터로 구조화합니다.
  - *의의*: 다중 절 구조와 카메라 담화가 엉켜 오분류를 만들던 규칙 충돌을 완전히 제거하고 확장성을 극대화했습니다.
* **Slide 3: [실전 추론] AI Score 수식화 및 4대 집계 추론 모드 서빙**
  - *핵심*: 최종 산량 점수(AI Score)에 따라 VLM이 텍스트 단서를 신뢰할지(Text-First), 시각적 미세 동작 변화에 초점을 맞출지(Visual-First)를 4대 라우팅 모드로 결정해 어텐션을 동적으로 튜닝합니다.
