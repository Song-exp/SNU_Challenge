...
**주력은 여전히 exp07(aug2, 48.41%, Public 0.766). 최대 발견 = 카메라 표현 유무 +27%p (holdout 실측). 7/17 낮: gemma 라벨링 train 전량 확장 가동 중(8.9초/문장, ~7/18 아침), 구조 파이프라인 인프라 완성(`Structure_Pipeline.ipynb` + exp15 준비 완료), 규정 정독(test 분석→학습 = 실격 조항 발견, §4e). 다음 액션 = 라벨링 완료 후 exp15 스모크→본학습 (`Structure_Pipeline.ipynb` ⓪부터).**
